...
class DataCfg(TypedDict):
    ...
    supp_idxs: Optional[
        Sequence[int]
    ]  # Indexes of the support frames to load. (default = (-1, 1))
    use_depth: Optional[
        bool
    ]  # If `True`, load ground truth depth maps. (default = False)
    use_hints: Optional[bool]  # If `True`, load proxy depth hints. (default = False)
    use_benchmark: Optional[
        bool
    ]  # If `True`, load corrected ground truth depth maps. (default = False)
    use_strong_aug: Optional[
        bool
    ]  # If `True`, use TrivialAugmentWide augmentations. (default = False)
    as_torch: Optional[
        bool
    ]  # If `True`, convert (x, y, meta) to torch. (default = True)
    use_aug: Optional[
        bool
    ]  # If `True`, apply augmentations. (default = False) (recommended = {core: True, val: False})
...
class LoaderCfg(TypedDict):
    ...
    num_workers: Optional[
        int
    ]  # Number of multiprocessing workers. (default = None) (recommended = num_cpus)
    drop_last: Optional[
        bool
    ]  # If `True`, drop last batch with inconsistent size. (default = False) (recommended = {core: True, val: True, test: False})
    shuffle: Optional[
        bool
    ]  # If `True`, randomly order dataset items. (default = False) (recommended = {core: True, val: False, test: False})
    pin_memory: Optional[
        bool
    ]  # If `True`, pin memory on GPU. (default = True) (recommended = True)
...
class TrainCfg(TypedDict):
    ...
    resume_training: Optional[
        bool
    ]  # If `True`, look for existing checkpoints and resume training. (default = False)
    load_ckpt: Optional[
        PathLike
    ]  # Path to load pretrained weights (does not resume training). (default = None)
    log_every_n_steps: Optional[int]  # Logging interval for scalars. (default = 100)
    monitor: Optional[
        str
    ]  # Metric to monitor when saving checkpoints & stopping. (default = 'AbsRel')

    benchmark: Optional[
        bool
    ]  # If `True`, set torch.backends.cudnn.benchmark. (default = False)
    gradient_clip_val: Optional[
        float
    ]  # Gradient norm clipping value. (default = None) (recommended = 1)
    precision: Optional[
        int
    ]  # Training batch gradients to accumulate. (default = 32) (recommended = 16)
    accumulate_grad_batches: Optional[
        int
    ]  # Training batch gradients to accumulate. (default = 1) (recommended = 1)

    swa: Optional[
        bool
    ]  # If `True`, enable Stochastic Weight Averaging callback. (default = False)
    early_stopping: Optional[
        bool
    ]  # If `True`, enable early stopping callback. (default = False)

    min_depth: Optional[
        float
    ]  # Minimum depth when scaling the network prediction (default = 0.1)
    max_depth: Optional[
        float
    ]  # Maximum depth when scaling the network prediction (default = 100)
