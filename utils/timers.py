...
class Timer:
    ...
    def __repr__(self) -> str:
        ...
        kwargs = {
            key: getattr(self, key) for key in sig.parameters if hasattr(self, key)
        }
...
class MultiLevelTimer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        timer["elapsed"] = round(
            self._sf * (timer["end"] - timer["start"]), self.precision
        )
        self.depth -= 1
    ...
    @staticmethod
    def mean_elapsed(
        timers: Sequence["MultiLevelTimer"],
    ) -> Union[Sequence, dict[str, float]]:
